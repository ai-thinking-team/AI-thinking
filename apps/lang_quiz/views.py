...
def _create_run(request, *, section, questions, mode='', course_slug='', source_name='', instruction=''):
    ...
    previous_created_at = (
        LanguageQuizRun.objects
        .filter(browser_session_key=browser_session_key)
        .order_by('-created_at')
    ...
    if previous_created_at is not None and run.created_at <= previous_created_at:
        run.created_at = previous_created_at + timedelta(microseconds=1)
        LanguageQuizRun.objects.filter(pk=run.pk).update(created_at=run.created_at)
