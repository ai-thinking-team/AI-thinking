...
JUDGE_SYSTEM_PROMPT = (
    ...
    '書いてください。学習者の考え方の説明（reasoning）が渡されている場合は、'
...
HINT_SYSTEM_PROMPT = (
    ...
    '4=解法の骨組み（最終的な計算は含まない部分的な解法）、5=完全な解答（他のヒントが'
    '不十分だった場合の最終手段）。指定されたレベルのヒントだけを、過不足なく作成して'
    'ください。レベルを超える情報（特に、指定レベルより上の段階でしか出さないはずの'
    '計算の続きや最終的な答え）は、たとえ親切のつもりでも絶対に含めないでください。'
