...
def generate_ai_response(
    *, system_prompt, user_prompt, response_schema=None, files=None
    ...
    Tries every configured provider in turn (OpenAI, then Gemini) and
    returns the first one that actually answers — a provider being
    *configured* (a key is present) doesn't mean it's currently
    *working* (out of quota, rate-limited, network trouble, ...), so a
    failure there falls through to the next provider instead of giving
    up. Set `files` to a list of `(file_bytes, mime_type)` pairs to attach
    one or more uploaded files (image, PDF, ...) alongside the prompt.
    """
    attempts = []
    if os.environ.get('OPENAI_API_KEY'):
        attempts.append(_generate_openai_response)
    if os.environ.get('GEMINI_API_KEY'):
        attempts.append(_generate_gemini_response)
